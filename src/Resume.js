...
function ReAchivement(props) {
    const sk = props.ob;
    const LisAch = sk.map((det, index) => {
        if (det[0].trim() === "") return null;
        if (det[1].trim() === "") return (<li key={index}>{det[0]}</li>);
        return (<li key={index}>{det[0]} (<a href={det[1]}>Link</a>)</li>);
    });
    if (sk.length === 0) return null;
    return (<><section>
        <h2>Achievements</h2>
    ...
    const Lispos = pos.map((det, index) => {
        ...
        return (
            <React.Fragment key={index}>
                <h3>
                    <b>{det.title}</b> (<a href={det.link}>Link</a>)
...
function Resume(props) {
    ...
    return (<>
        <div id="resume" className="resume" style={resSty}>
            ...
            <ReAchivement ob={d.lachievements} />
