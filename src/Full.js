...
class FullFun extends React.Component {
  ...
  setAchievement = (e) => {
    ...
    var prevState = this.state;
    var classIndex = showIndex(e.target, "f");
    var temp = document.getElementsByClassName("fAchivTit");
    var val = temp[classIndex].value;
    prevState.lachievements[classIndex][0] = val;
    temp = document.getElementsByClassName("fAchivLink");
    val = temp[classIndex].value;
    prevState.lachievements[classIndex][1] = val;
    this.setData(prevState);
  }
  setPositions = (e) => {
    e.preventDefault();
    var prevState = this.state;
    var classIndex = showIndex(e.target, "f");
    var classList = ["fTitle", "fTimeline", "fPosLink"];
    classList.forEach(className => {
      var temp = document.getElementsByClassName(className);
      var val = temp[classIndex].value;
      var nam = temp[classIndex].name;
  ...
  newForEdu = (e) => {
    e.preventDefault();
    var prevState = { ...this.state };
    prevState.education.unshift({ startYear: "", endYear: "", degree: "", institution: "", CGPA: "", percentage: "" });
    this.setData(prevState);
  }
  ...
  newForExperience = (e) => {
    e.preventDefault();
    var prevState = { ...this.state };
    prevState.experience.unshift({ company: "", link: "", date: "", role: "", achievements: ["", ""] });
    this.setData(prevState);
  }
  ...
  newForPosition = (e) => {
    e.preventDefault();
    var prevState = this.state;
    prevState.positions.unshift({
      title: "", timeline: "", achievements: ["", ""], link: ""
    });
    this.setData(prevState);
  }
  generatePDF() {
    ...
    return (
      <>
        <div className='FormInter'>
          ...
          <form id="FormIdFull" onSubmit={this.handleSubmit} >
              ...
              <div className='spliFor'>
                ...
                <div className='spliChd'>
                  <div className='divCheBo'>
                    <input type="checkbox" onChange={this.togData} checked={this.state.preview} />
                  </div>
                </div>
              </div>
            </fieldset>
            <div>
              <fieldset>
                ...
                <br />
                <h1>Advised to save as text file for Custom Upload.</h1>
              </fieldset>
            </div>
            ...
            <div id="divForAchiv">
              ...
              {this.state.lachievements.map((con, index) => {
                return (<NewForAchievement rem={this.remove} cha={this.setAchievement} base={this.state.lachievements} ind={index} key={index}></NewForAchievement>);
              })}
            </div>
...
function NewForAchievement(props) {
  ...
  return (
    <div className='divSkillsSer'>
      <fieldset>
        <legend>Your {numbering} Achievement</legend>
        <span onClick={() => { rem(base, ind) }} className="remBut">&#10060;</span>
        <input id={"AchiTit-" + ind} name="AchieveTit" className="fAchivTit" onChange={cha} value={base[ind][0]} type="text" placeholder=" " />
        <label htmlFor={"AchiTit-" + ind}>Achievement Text</label>
        <input id={"AchiDes-" + ind} name="AchieveDes" className="fAchivLink" onChange={cha} value={base[ind][1]} type="text" placeholder=" " />
        <label htmlFor={"AchiDes-" + ind}>Achievement Link</label>
      </fieldset>
    </div>
  );
}
  ...
  return (
    <div className='divPosSer'>
      <fieldset>
        ...
        <input id={"PosLink-" + ind} name="link" className="fPosLink" onChange={cha} value={base[ind].link} type="text" placeholder=" " />
        <label htmlFor={"PosLink-" + ind}>Certificate Link</label>
        {achi}
        <button type='button' onClick={() => { achiAdd(base, ind) }} className="fAddExpAchi addBut">+ Add Achievement</button>
